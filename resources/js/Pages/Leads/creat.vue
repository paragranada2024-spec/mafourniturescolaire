  ...
  <div class="max-w-xl mx-auto p-8 bg-white shadow-md  rounded mt-24">
    ...
    <form @submit.prevent="submit">
      ...
      <div class="mb-4">
        ...
        <input v-model="form.phon" type="text" class="w-full border rounded px-4 py-2 mt-1" />
        <span class="text-red-500 text-sm" v-if="form.errors.phon">{{ form.errors.phon }}</span>
      </div>
      ...
      <button  type="button"
      @click="goBackToLead"
            class="  text-sm text-indigo-600 hover:underline font-medium ">⬅️ Retour aux List</button>
            </div>
          </form>
  </div>
</template>
